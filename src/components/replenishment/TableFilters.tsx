    ...
    <section className="glass-panel rounded-3xl p-5">
      <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => onChange({...filters, risk: filters.risk === "High" && filters.finalPositive === "no" ? "" : "High", finalPositive: filters.risk === "High" && filters.finalPositive === "no" ? "" : "no"})} className={`rounded-full border px-3 py-2 text-xs font-semibold transition ${filters.risk === "High" && filters.finalPositive === "no" ? "border-amber-300/35 bg-amber-400/14 text-amber-100" : "border-white/10 bg-white/[0.04] text-cockpit-muted hover:bg-white/[0.07]"}`}>High risk · zero qty</button>
